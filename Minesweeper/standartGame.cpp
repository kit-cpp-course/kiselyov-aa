...
void standartGame::introduction()
{
	...
	else if (com == "l")
	{
		...
		else
		{
			...
			if (fiel) standartGame::currentTurn(fiel);
		}
	}
}

void standartGame::startGame()
{
	string size;
	string minesCount;
	...
	f->plantBombs();
	standartGame::currentTurn(f);
}

void standartGame::currentTurn(field * f)
{
	system("cls");
	HANDLE hStdOut = GetStdHandle(STD_OUTPUT_HANDLE);
	SetConsoleTextAttribute(hStdOut, (WORD)(0 | 2));
	...
	case 0:
	{
		string cell = standartGame::suitableCell(f);
		unsigned int j = cell[0] - 65;
		unsigned int i = atoi(cell.erase(0, 1).c_str());
		standartGame::markCell(f, i, j);
		break;
	}
	case 1:
	{
		...
		unsigned int j = cell[0] - 65;
		unsigned int i = atoi(cell.erase(0, 1).c_str());
		standartGame::openCell(f, i, j);
		break;
	}
	case 2:
	{
		standartGame::saveGame(f);
		standartGame::ifContinue(f);
		break;
	}
	case 3:
	{
		standartGame::loadOrDeleteSavedGame(f);
		break;
	}
	case 4:
	{
...
void standartGame::markCell(field *f, unsigned int i, unsigned int j)
{
	HANDLE hStdOut = GetStdHandle(STD_OUTPUT_HANDLE);
	f->changeState(i, j, 1);
	if (f->ifVictory())
...
void standartGame::openCell(field *f, unsigned int i, unsigned int j)
{
	HANDLE hStdOut = GetStdHandle(STD_OUTPUT_HANDLE);
	...
	{
		f->changeState(i, j, 2);
		if (f->minesCountAroundCell(i, j) == 0) standartGame::openCellsAroundZeroCell(i, j, f);
		if (f->ifVictory())
		{
			system("cls");
			cout << endl;
...
void standartGame::saveGame(field *f)
{
	ofstream fout;
...
string standartGame::encryption(unsigned int * bombs, field *f)
{
...
string standartGame::decryption(string str)
{
	str += " ";
	string newStr = " ";
	unsigned int num = 11;
...
unsigned int standartGame::changeNum(unsigned int num)
{
	if (num == 11) num = 17;
...
bool standartGame::ifStringIsInt(string str)
{
...
bool standartGame::checkCellNumber(field * f, string cell)
{
	unsigned int size = f->returnSize();
	char letter = cell[0];
...
void standartGame::openCellsAroundZeroCell(unsigned int x, unsigned int y, field * f)
{
	unsigned int size = f->returnSize();
...
unsigned int standartGame::markedCellsCount(field * f)
{
	int count = 0;
	unsigned int size = f->returnSize();
	for (unsigned int i = 0; i < size; i++)
